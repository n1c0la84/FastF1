...
# Enable cache
fastf1.Cache.enable_cache('cache')  # change to unique cache location
...
for driver in driver_list:  # Gets telemetry for all drivers in list
    ...
    driver_telemetry['Colour'] = driver_year_color(laps.pick_driver(driver).pick_fastest()['Driver'],
                                                   session.weekend.year)
...
fastest_minisector = fastest_minisector[['Minisector', 'Driver', 'Speed']].rename(
    columns={'Speed': 'Sector_avg_speed', 'Driver': 'Fastest_driver'})
...
for driver in driver_list:
    telemetry.loc[telemetry['Fastest_driver'] == driver, 'Fastest_driver_int'] = int(
        drv_num[np.where(drivers == driver)])
...
for driver in minisector_fastest_drivers:
    minisector_colours.append(
        driver_year_color(laps.pick_driver(driver).pick_fastest()['Driver'], session.weekend.year))

minisector_colours = np.array(minisector_colours)

minisector_details = pd.DataFrame(
    {'Fastest_driver': minisector_fastest_drivers, 'Fastest_driver_int': minisector_fastest_drivers_num,
     'Minisector_colours': minisector_colours}, columns=['Fastest_driver', 'Fastest_driver_int', 'Minisector_colours'])
...
plt.title((driver_list[0] + ' vs ' + driver_list[1] + ' ' + str(session.weekend.year) + ' ' + str(
    session.weekend.name) + ' ' + str(session.name) + '\n' + 'Number of minisectors = ' + str(num_minisectors)),
          fontsize=20)
...
fastest_minisector_xy = fastest_minisector_xy[['Minisector', 'Driver', 'Speed', 'X', 'Y']].rename(
    columns={'Speed': 'Sector_avg_speed', 'Driver': 'Fastest_driver', 'X': 'X_avg', 'Y': 'Y_avg'})

# Rename the driver column
fastest_drivers_xy = fastest_drivers_xy[['Minisector', 'Driver', 'Speed', 'X', 'Y']].rename(
    columns={'Driver': 'Fastest_driver', 'X': 'X_avg', 'Y': 'Y_avg'})

############################################################################################################################################


for driver in fastest_minisector_xy['Fastest_driver']:
    fastest_minisector_xy.loc[fastest_minisector_xy['Fastest_driver'] == driver, 'Fastest_driver_int'] = int(
        drv_num[np.where(drivers == driver)])
